<?php
/*
collapsing categories version: 2.1.3
    ...
    if (in_array($cat->term_id, $parents)) {
        foreach ($categories as $cat2) {
            ...
            if ($cat->term_id == $cat2->parent) {
                ...
                list($subCatPostCount2, $posttext2) = getSubPosts(isset($postsInCat[$cat2->term_id]) ? $postsInCat[$cat2->term_id] : array(), $cat2, $showPosts);
                    ...
                    if (empty($cat2->description)) {
                        $link2 .= 'title="' .
                            sprintf(__("View all posts filed under %s",
                                'collapsing-categories'),
                                esc_html(apply_filters('single_cat_title', $cat2->name))) . '"';
                    } else {
                        $link2 .= 'title="' .
                            esc_html(apply_filters('description',
                ...
                // error_log('$collapsCatItems='.json_encode($collapsCatItems, JSON_PRETTY_PRINT));
                if ($theID != '' && !isset($collapsCatItems[$theID])) {
...
function collapscat_catfilter($categories, $taxonomy, $args) {
    if (!isset($args['plugin_guard'])) {
        return $categories;
    }
    global $options;
    extract($options);
...
function get_collapscat_fromdb($args = '') {
    ...
    if ($showPosts) {
        ...
        foreach ($posts as $post) {
            // error_log('$post->term_id' . json_encode($post, JSON_PRETTY_PRINT));
            if (empty($postsInCat[$post->term_id])) {
                $postsInCat[$post->term_id] = array();
            }
            array_push($postsInCat[$post->term_id], $post);
        }
        // error_log('$postsInCat='.json_encode($postsInCat, JSON_PRETTY_PRINT));
    }
    add_filter('description', 'collapscat_replace_newlines');
    add_filter('get_terms', 'collapscat_catfilter', 10, 3);
        ...
        if ($theCount > 0 || $showEmptyCat) {
            ...
            if (empty($cat->description)) {
                $link .= 'title="' .
                    sprintf(__("View all posts filed under %s",
                        'collapsing-categories'),
                        esc_html(apply_filters('single_cat_title', $cat->name))) . '"';
            } else {
                $link .= 'title="' . esc_html(apply_filters(
